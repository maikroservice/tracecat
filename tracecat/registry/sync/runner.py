...
class RegistrySyncRunner:
    ...
    async def _resolve_sandboxed_package(
        ...
        if request.git_url.startswith("git+https://"):
            # The NsJail clone jail only supports SSH targets; clone HTTPS
            # origins with the runner's git (token via GIT_ASKPASS) while
            # install and discovery stay sandboxed. Token is optional: public
            # repositories work anonymously.
            logger.warning(
                "Registry HTTPS Git clone runs outside the clone jail",
                origin=request.origin,
            )
            token = await self._fetch_registry_git_token(request.organization_id)
