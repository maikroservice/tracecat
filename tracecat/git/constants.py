...
GIT_PLUS_HTTPS_URL_REGEX = re.compile(
    r"^git\+https://(?P<host>[^/:@]+)(?::(?P<port>\d+))?/(?P<path>[^/@]+?(?:/[^/@]+?)+?)(?:\.git)?(?:@(?P<ref>[^@]+))?$"
)
"""Pip-style git+https URL with optional port and multi-segment paths. Credentials in the netloc are rejected."""
