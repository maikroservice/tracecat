...
class GitSettingsUpdate(BaseSettingsGroup):
    ...
    def validate_git_repo_url(cls, value: str | None) -> str | None:
        """Validate that git_repo_url is a valid git+ssh or git+https URL if provided."""
        if value is None:
            return value

        # Use shared regexes from git utils to ensure consistency across the codebase
        if not (
            GIT_SSH_URL_REGEX.match(value) or GIT_PLUS_HTTPS_URL_REGEX.match(value)
        ):
            raise ValueError(
                "Must be a valid Git URL (e.g., git+ssh://<user>@github.com/org/repo.git "
                "or git+https://gitlab.com/org/repo.git)"
            )
